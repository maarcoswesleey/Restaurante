...
if($total_reg > 0){ 
	...
	if($total_reg > 0){ 
		...
		$totalVendasM = 0;
		$query = $pdo->query("SELECT * from reservas where data = curDate()");
		$res = $query->fetchAll(PDO::FETCH_ASSOC);
...
<div id="principal">
    <p>Reservas no Ano de <?php echo $ano_atual ?></p>
<?php
...
$totalVenM = 0;
		$query = $pdo->query("SELECT * from reservas where data >= '$dataMesInicio' and data <= '$dataMesFinal'");
		$res = $query->fetchAll(PDO::FETCH_ASSOC);
