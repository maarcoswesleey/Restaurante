...
if($total_reg > 0){ 
	...
	if($total_reg > 0){ 
		...
		$totalVendasM = 0;
		$query = $pdo->query("SELECT * from reservas where data >= '$dataInicioMes' and data <= '$dataMesFinal'");
		$res = $query->fetchAll(PDO::FETCH_ASSOC);
		...
		$total_pedidos = 0;
		$query = $pdo->query("SELECT * from pedidos where data >= '$dataInicioMes' and data <= '$dataMesFinal'");
		$res = $query->fetchAll(PDO::FETCH_ASSOC);
		$total_ped = @count($res);
		if($total_ped > 0){ 
			...
			<section id="minimal-statistics">
				<div class="row mb-2">
					<div class="col-12 mt-3 mb-1">
						<h4 class="text-uppercase">Algumas Estatísticas</h4>
				...
				<section id="stats-subtitle">
					<div class="row mb-2">
						<div class="col-12 mt-3 mb-1">
							<h4 class="text-uppercase">Detalhamento de Vendas</h4>
